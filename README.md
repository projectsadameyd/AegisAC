...
Severe Speed movement is rejected on the first implausibly large non-exempt movement event. Two spaced severe attempts within five seconds advance one sanction stage, subject to a separate 15-second severe sanction cooldown. Aegis computes a loose movement envelope from the player’s current speed attributes, elapsed movement time, and preceding movement. This is a limited event-level estimate, not packet-level simulation of all possible Minecraft movement. Teleports, recent server velocity or damage, special surfaces, high ping, and low TPS are exempt. Moderate Speed and other checks keep their regular evidence windows. These thresholds need testing on the actual server and can still miss cheats or affect legitimate players.
...
| `/aegis enforcement cooldown <0-3600>` | Set the seconds between sanction stages; 0 is for controlled testing |
...
If a check appears idle, inspect `/aegis debug <player>` while that player is moving and interacting. It separates total movement events from **evaluated**, **exempt**, and **rejected severe** positional movements. Alert count, last heuristic score, the sanction gate, current stage, cooldown, and next action explain why a failed sample did not cause a kick or ban. After eight qualifying Speed alerts spanning at least five seconds, the first sanction is a kick; stages persist between reconnects. Check `/aegis info` for the loaded plugin version and `/aegis status` for sanction settings. The CI build tests the sustained-alert and severe-movement thresholds; it does not replace testing on a running Paper server.

### If nothing happens on your server

1. Remove older AegisAC JARs from `plugins/`, install only `AegisAC-0.3.5-alpha.jar`, restart Paper, and check `/aegis info` or `/aegis status` for `0.3.5-alpha`. A config reload cannot replace loaded Java code.
...
3. Trigger activity while checking `/aegis debug <online-player>` and console `[AUDIT] ALERT` or `[AUDIT] SANCTION` entries. The `gate=` field names the last reason a sanction did not advance. `stage=1` means the first kick worked; stages 1–3 are kicks, and stage 4 is the first temporary ban. After each regular sanction the default cooldown is 10 minutes; severe Speed evidence uses 15 seconds by default. In a private test, run `/aegis enforcement cooldown 0` to immediately allow the next qualifying evidence sequence; restore `/aegis enforcement cooldown 600` afterward. The command takes effect on existing cooldowns. The next sanction is still a kick until stage 4. `/aegis status` warns when a configured minimum score exceeds a check's possible score.
